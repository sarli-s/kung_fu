...
class ChessBoardRules(BoardRules):
    def prepare_piece(self, token, piece, board, fmt):
        if fmt.piece_type(token) == "P":
            piece.mover.start_row = (board.rows() - 1) if fmt.color(token) == "w" else 0

    def get_promotion(self, token, to_row, board, fmt):
        if fmt.piece_type(token) != "P":
            return None
        last_row = 0 if fmt.color(token) == "w" else board.rows() - 1
        if to_row == last_row:
            return fmt.encode(fmt.color(token) + "Q")
        return None

    def is_royal(self, token, fmt):
        return fmt.piece_type(token) == "K"
